...
★ 단 이 2층 모델이 보장하는 것은 **"릴레이는 지시를 지어낼 수 없다"** 까지다.
사용자가 여럿이면 릴레이는 *지어내는 대신 **엉뚱한 사람에게 배달**할* 수 있다 —
A 에게 갈 진짜 서명된 진입을 B 에게 주면 서명 검증은 통과한다.
그래서 클라는 **자기 `acct` 가 아닌 봉투를 거절한다**(`E_ACCT`, `docs/protocol.md §2.4`).
